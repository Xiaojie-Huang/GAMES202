        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    //该像素所代表的矩形区域投影到单位球面的面积
                    float w = CalcArea(x,y,width,height);

                    //投影操作,用前SHOrder阶的球谐函数
                    for(int l = 0; l <= SHOrder; ++l)
                    {
                        //每阶球谐函数的编号-l——l
                        for(int m = -l;m <= l;++m)
                        {
                            int n = sh::GetIndex(l,m);
                            double BasicFunc = sh::EvalSH(l,m,dir.cast<double>().normalized());

                            //对于CubeMap中的每一处光线，进行累加来近似积分
                            SHCoeffiecents[n] += Le * w * BasicFunc;
                        }
                    }
                }
            }
